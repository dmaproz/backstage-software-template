...
- **Host:** `${{values.app_name}}.parcer.com`
- **Path:** `/app/`
- **Path Type:** Prefix
- **Backend Service:** `${{values.app_name}}:8080`
...
- **Internal:** `http://${{values.app_name}}:8080`
- **External:** `http://${{values.app_name}}.parcer.com/app/` (via Ingress)
...
The Helm chart is located in `charts/${{values.app_name}}/`:
...
- **Ingress Host:** `${{values.app_name}}.parcer.com`
- **Ingress Class:** nginx
- **Resources:**
  - CPU Request: 50m
  - Memory Request: 50M
...
helm install ${{values.app_name}} ./charts/${{values.app_name}} -f custom-values.yaml

# Upgrade existing installation
helm upgrade ${{values.app_name}} ./charts/${{values.app_name}}

# Uninstall
helm uninstall ${{values.app_name}}
```
...
replicaCount: 3
image:
  repository: your-registry/${{values.app_name}}
  tag: latest
service:
  type: LoadBalancer
  port: 5000
...
- **Component Name:** ${{values.app_name}}
- **Type:** service
- **Owner:** capes
